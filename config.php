...
/*
If the account is affected by feedback_required, just wait 24 hours later to recover.
If you raun the Cronjob, set to 5 Minutes or whatever.
*/

// YOU CAN ONLY CHANGE THIS PART
$total = 2; // Total media that you want to comment on | so when the script is run, it only takes a few media, don't input too much to avoid feedback
$first_koment = false; // Only comments on the media that have not been commented on | true/false
$komentar = 'komentar.txt'; // Your Comments List
$saveFile = 'logData.txt'; // File log
$cookieFile = 'cookieData.txt'; // File cookie
$delay = 3; // Comments Delay per media(SECOND)
